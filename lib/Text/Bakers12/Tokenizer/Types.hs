...
--
-- The implementation of this could use fold, but I thought that using
-- `T.concat` and not constructing any intermediate Tokens would be faster.
concat :: [Token] -> Token
concat []       = empty
concat [t]      = t
...
concat ts@(a:_) =
    ...
    where
        ...
        -- three times. (Best would be to write `unzipWith3` or something,
        -- which would insure that the list was only walked twice. Someday.)
